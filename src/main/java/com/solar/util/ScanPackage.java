...
public class ScanPackage {
    ...
    public Set<Class<?>> getClassBean(String packName) {
        Set<Class<?>> classBeans = new LinkedHashSet<Class<?>>();
        Set<Class<?>> classes = getClass(packName);
        if (classes != null && classes.size() > 0) {
            for (Class<?> classBean : classes) {
                if (classBean.getAnnotation(SolarHandler.class) != null) {
    ...
    private Set<Class<?>> getClass(String pack) {
                        ...
                        if (name.startsWith(packageDir)) {//如果前半部分和定义的包名相同
                            ...
                            if (idx != -1 || recuresive) {
                                if (name.endsWith(".class") && !entry.isDirectory()) {
                                    String className = name.substring(pack.length() + 1,
                                        name.length() - 6);//去掉后面的“.class”，获取真正的类名
                                    try {
    ...
    private void findAndClassesInPackageByFile(String packageName, String packagePath,
        ...
        for (File file : dirfiles) {
            if (file.isDirectory()) {
                findAndClassesInPackageByFile(packageName + "." + file.getName(),
                    file.getAbsolutePath(), recuresive, classes);
            } else {
    ...
    public static void main(String[] args) throws IllegalAccessException {
        ...
        for (Class<?> obj : classes) {
            ...
            //				}
            //			}
            if (obj.getAnnotation(SolarHandler.class) != null) {
